 ...
 * @author gideon
 */
public final class AnnotatedPojoInterfaceTable extends AnnotatedPojoTable {
     ...
     */
    public AnnotatedPojoInterfaceTable(Database database, Class<?> pojoClass, Set<AnnotatedPojoTable> subTables) {
    ...
    public List<Field> getFields() {
        if (fieldModels == null) {
            List<java.lang.reflect.Method> fields = getAllFields(getObjectClass());
            fieldModels = new ArrayList<>();
            for (java.lang.reflect.Method field : fields) {
                if (isDataField(field)) {
                    fieldModels.add(new AnnotatedMethodField(this, field));
                }
            }
        }
        return fieldModels;
     ...
     */
    private boolean isDataField(java.lang.reflect.Method field) {
     ...
     */
    private List<java.lang.reflect.Method> getAllFields(Class<?> clazz) {
        List<java.lang.reflect.Method> res = new LinkedList<>();
        for (Class<?> sup : clazz.getInterfaces()) {
            res.addAll(getAllFields(sup));
        }
        res.addAll(Arrays.asList(clazz.getDeclaredMethods()));
        return res;
